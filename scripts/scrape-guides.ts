...
async function main(): Promise<void> {
    ...
    const currentUrls = new Set(links.map((l) => normalize(l.url)));
    const removedUrls = Object.keys(meta.guides).filter((u) => !currentUrls.has(normalize(u)));
    ...
    // Phase 5: NLM sync (strictly opt-in via --sync-nlm <notebook-id>)
    const nlmNotebook = SYNC_NLM_NOTEBOOK;
    if (nlmNotebook) {
        meta.nlmNotebookId = nlmNotebook;
        await saveMeta(meta);
        if (stats.newCount > 0) {
