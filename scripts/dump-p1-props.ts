...
async function dumpRowsAndCells(pageId: string) {
    ...
    for (const [key, val] of blocks.entries()) {
        if (val instanceof Y.Map) {
            ...
            // Show rows, text cells (for vertical align), and tableText blocks
            if (type === "row" || type === "tableCellText" || type === "tableText") {
                console.log(`\n${type} "${key}":`);
                for (const [k, v] of m.entries()) {
                    if (v instanceof Y.Map) {
                        ...
                        console.log(`  ${k}: Y.Text(${JSON.stringify(v.toDelta())})`);
                    } else {
...
await dumpRowsAndCells("I1XIyTUrhQMTDaJE");
