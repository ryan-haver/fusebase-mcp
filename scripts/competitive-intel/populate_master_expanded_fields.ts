...
async function main() {
  ...
  // Define intelligence mappings based on segments & architectures
  ...
  for (const row of rows) {
    ...
    const name = row.OtiwIUpQ || row.UmOYNqou || '';
    const segment = row.ACIiaI_q || 'AI-Native';
    const arch = row.RfkVSRYK || 'Orchestration Overlay';

    // Infer Model Strategy
    let modelStrategy = 'Frontier LLM Router';
    if (arch.includes('In-Place') || arch.includes('Federated')) {
      modelStrategy = 'Hybrid Local+Cloud';
    } else if (name.includes('Crogl') || name.includes('Mate') || name.includes('Vega') || name.includes('Qevlar')) {
      modelStrategy = 'Proprietary Fine-Tuned SLMs';
    } else if (segment === 'Legacy Platform') {
      modelStrategy = 'BYO-LLM';
    }

    // Infer Pricing Metric
    let pricingMetric = 'Outcome / Ticket Resolved';
    if (segment === 'Legacy Platform') {
      pricingMetric = 'Ingested GB/TB';
    } else if (segment === 'AI-Adjacent') {
      pricingMetric = 'Per-Seat / Analyst';
    } else if (name.includes('Dropzone') || name.includes('7AI') || name.includes('Simbian')) {
      pricingMetric = 'Flat Platform Tier';
    }

    // Infer Deployment Mode
    let deploymentMode = ['Multi-Tenant SaaS', 'Dedicated VPC'];
    if (arch.includes('In-Place') || name.includes('Crogl') || name.includes('Vega') || segment === 'Legacy Platform') {
      deploymentMode = ['Dedicated VPC', 'Customer Cloud On-Prem', 'Air-Gapped / Sovereign'];
    }
    ...
    updatesToApply.push({
      ...
      values: [
        { item_key: 'vz07PLSM', value: modelStrategy },
        { item_key: 'fnrh-uFt', value: pricingMetric },
        { item_key: '3AsrcUS5', value: deploymentMode },
      ],
    });
  }

  console.log('\n--- Step 2: Batch updating master rows with Model Strategy, Pricing & Deployment ---');
  const BATCH_SIZE = 15;
