...
| ID | Finding | Status | Fix |
|---|---|---|---|
| DOC-1 | README and badges claim "100% verified" and give inconsistent counts (165/168/175 tools, 206/212 assertions) | ✅ fixed (2026-09-25; marked done earlier, but the dashboard's test data, the status resource and two badges were still hand-written) | `test-all` records each stage's outcome and counts in `.cache/test-results.json`; `generate-status-data.ts` publishes only those; `fusebase://status` carries tool counts (checked against the registry by a unit test) and no test figures; README badges and headline state only what the tests prove |
